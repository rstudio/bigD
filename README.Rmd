...
The goal of i18nExtras is to easily get localized variants of dates, times, currencies, and other stuff that can be localized with some degree of precision.
...
We can also apply a smattering of different simple date format presets to this input. The `fmt_date_time()` function is useful for this. We can choose to keep the date portion, the time portion, or both. The `fdf()` helper function is useful for choosing a preset date format from the flexible date format (FDF).
...
You can install **i18nExtras** from GitHub with:

``` r
remotes::install_github("rich-iannone/i18nExtras")
```
